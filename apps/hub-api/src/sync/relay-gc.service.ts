...
// referenced it whenever a canonical write loses a race — a newer push
// supersedes it (last-write-wins), leaving the previous blob pointed at by
// nothing. Those orphans are harmless but accumulate forever without a
// sweep — this service reclaims them per-user.
@Injectable()
export class RelayGcService {
  ...
  // Gather every blob hash still referenced for `userId` — the canonical hash
  // of every file_state row in the user's projects — and delete every stored
  // blob not in that set.

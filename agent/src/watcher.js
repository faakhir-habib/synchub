...
export function watchProjects(api, state, mappings, log = () => {}, debounceMs = 1500) {
    ...
    const onChange = (path) => {
      ...
      timers.set(path, setTimeout(async () => {
        try {
          ...
          await pushLocal(api, state, m.project_id, m.local_path, fn, content, state.get(m.project_id, fn), log);
        } catch (e) {
